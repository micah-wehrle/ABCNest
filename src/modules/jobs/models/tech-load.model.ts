export class TechLoad {
  ...
  private generateJob(): JobData {
    ...
    const jobType: string = this.randOf(['Install', 'Repair', 'Helper', 'POTS', 'BSW']);
    const transportType: string = jobType === 'POTS' || this.nextRand() < 0.5 ? this.randOf(['IP-CO', 'IP-RT', 'FTTN', 'FTTN-BP']) : this.randOf(['FTTP GPON', 'FTTP XGS-PON']);
  ...
  private generateEmail(first: string, last: string): string {
    ...
    let email = '';
    email += this.nextRand() < 0.1 ? `${this.randOf(['the', 'super', 'dr', 'smiling'])}.` : '';
  ...
  private generateServices(transportType: string): Services {
    ...
    services.equipment.push(isFiber && !has010 ? '320' : this.randOf(['210', '5268', '599', '589']));

    if (this.nextRand() < 0.15) { // add uverse 
      services.uversePackage = this.randOf(['U100', 'U200', 'U300']); // Micah doesn't really know uverse stuff too well. Feel free to adjust any uverse arrays below!
      const uverseBoxCount = Math.floor( (this.nextRand()*6 + this.nextRand()*6) / 2);
      services.equipment.push(this.randOf(['2262v2', '9001', '2200']));
      for (let i = 0; i < uverseBoxCount; i++) {
        services.equipment.push(this.randOf(['2500', '7005', '1200']));
      }
    }
